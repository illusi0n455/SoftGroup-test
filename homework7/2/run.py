...
from app.models import db, User, Theme, lower
...
def detail(id):
    ...
    """
    topic = Theme.query.filter_by(id=id).first_or_404()
    detail = {'id': id, 'title': topic.title, 'author': topic.author, 'url': topic.url, 'text': topic.text,
              'price': topic.price, 'currency': topic.currency}
...
def home_page_post():
    ...
    if request.method == 'POST':
        ...
        if search_query:
            ...
            topics = Theme.query.filter((Theme.title.ilike(like_str)) | (Theme.author.ilike(like_str)))
